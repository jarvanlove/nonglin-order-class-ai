"""Strict overflow check v4 - safe slide activation."""
...
with sync_playwright() as p:
    ...
    for day in DAYS:
        ...
        for i in range(total):
            # Safe activation: directly toggle active class by index
            page.evaluate(f"document.querySelectorAll('.slide').forEach((s,idx) => s.classList.toggle('active', idx === {i}));")
        ...
        if bad:
            ...
            if nocontent:
                print(f"[{day}] {len(nocontent)}/{total} no-content: {[b[0] for b in nocontent]}")
        else:
